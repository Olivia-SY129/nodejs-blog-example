...
router.get('/', async (req, res) => {
  if (req.user) {
    ...
    const posts = (await postsCursor.toArray()).map(
      ({ users, createdAt, ...rest }) => ({
        ...rest,
...
router.get('/reset-password', async (req, res) => {
  ...
  redirectWithMsg({
    res,
    dest: '/',
    info: '비밀번호가 변경되었습니다. 해당 비밀번호로 로그인해주세요.',
  })
})
...
router.post('/request-reset-password', async (req, res) => {
  ...
  await ses
    .sendEmail({
      ...
      FromEmailAddress: 'noreply@myfcblog.com',
    })
    .promise()
...
router.post('/signup', async (req, res) => {
  ...
  await ses
    ...
    .promise() // ses는 끝에 promise를 붙여주어야 함.
